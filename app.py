...
st.markdown("""
    ...
    .main-header {font-size: 2.4rem; font-weight: 700; color: #2563EB; margin-bottom: 5px; display: flex; align-items: center;}
    .sub-header {font-size: 1.1rem; font-weight: 600; color: #3B82F6; margin-bottom: 25px;}
    .stButton>button {background-color: #3B82F6; color: white; border-radius: 8px; font-weight: 600; height: 50px; transition: all 0.3s ease; border: none; box-shadow: 0 4px 6px -1px rgba(59, 130, 246, 0.4);}
    .stButton>button:hover {background-color: #2563EB; color: white; box-shadow: 0 10px 15px -3px rgba(37, 99, 235, 0.5); transform: translateY(-2px);}

    /* Modern Metric Cards */
    .metric-card {background: #FFFFFF; padding: 24px; border-radius: 12px; border: 1px solid #BFDBFE; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.05); text-align: center;}
    .metric-title {font-size: 0.95rem; font-weight: 700; color: #3B82F6; text-transform: uppercase; letter-spacing: 0.05em; margin-bottom: 8px;}
    .metric-value {font-size: 2rem; font-weight: 700; color: #10B981; margin: 0;}
    .metric-highlight {color: #059669;}

    .section-title {font-size: 1.5rem; font-weight: 700; color: #2563EB; margin-top: 30px; margin-bottom: 15px; border-bottom: 2px solid #BFDBFE; padding-bottom: 5px;}
    </style>
            ...
            try:
                ...
                <div style='display: flex; gap: 20px; flex-wrap: wrap; margin-bottom: 20px;'>
                    <div class='metric-card' style='flex: 1; min-width: 200px;'>
                        <div class='metric-title'>{parking_code} Subtotal</div>
                        <div class='metric-value'>{fmt_currency(subtotal)}</div>
                    </div>
                    <div class='metric-card' style='flex: 1; min-width: 200px;'>
                        <div class='metric-title'>External Rev</div>
                        <div class='metric-value'>{fmt_currency(net_calc + sh_sum)}</div>
                    </div>
                    <div class='metric-card' style='flex: 1; min-width: 200px;'>
                        <div class='metric-title'>Enforcement Rev</div>
                        <div class='metric-value'>{fmt_currency(Enforce)}</div>
                    </div>
                    <div class='metric-card' style='flex: 1; min-width: 200px; border-bottom: 4px solid #10B981;'>
                        <div class='metric-title'>Total Payout ({client_pct}% Split)</div>
                        <div class='metric-value metric-highlight'>{fmt_currency(payout)}</div>
                    </div>
                </div>
                """, unsafe_allow_html=True)

                st.markdown("<h5 style='font-weight: 700; color: #2563EB; margin-top: 20px;'>Detailed Summary Breakdown</h5>", unsafe_allow_html=True)
                ...
                st.dataframe(main_summary.style.format({"Value": "${:,.2f}"}), use_container_width=True)
                ...
                with pd.ExcelWriter(buffer, engine='openpyxl') as writer:
                    ...
                    if parking_code == 'BCW' and 'bcws_all_rows' in locals() and not bcws_all_rows.empty:
                        ...
                        sub_summary_data = {'Metric': ['Sum of Total Amount', 'Sum of Payment Gateway Fee', 'Total (Sum of Total - Sum of Gateway)', 'Less Tech Fee (10% of Total)', 'Net (Total - Less Tech Fee)'], 'Value': [float(bcws_all_rows['Total Amount'].sum()), float(bcws_all_rows['Payment Gateway Fee'].sum()), total_calc, float(total_calc * 0.10), net_calc]}
