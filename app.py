...
st.markdown("""
    <style>
    .main-header {font-size: 2.8rem; font-weight: 700; color: #1E3A8A; margin-bottom: 0px;}
    .sub-header {font-size: 1.2rem; font-weight: 400; color: #6B7280; margin-bottom: 20px;}
    .stButton>button {background-color: #2563EB; color: white; border-radius: 8px; font-weight: bold; height: 50px;}
    .stButton>button:hover {background-color: #1D4ED8; color: white;}
    .metric-card {background-color: #F8FAFC; padding: 20px; border-radius: 10px; border-left: 5px solid #2563EB; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.1);}
    </style>
""", unsafe_allow_html=True)

# --- Main Dashboard ---
st.markdown("<div class='main-header'>🚗 Slice Parking Payout Generator</div>", unsafe_allow_html=True)
st.markdown("<div class='sub-header'>Generate targeted payout reports based on specific parking codes.</div>", unsafe_allow_html=True)
...
with st.sidebar:
    ...
    if parking_code == "HBH":
        main_file = st.file_uploader("1. Main Transaction Data (HBH Slice Parking)", type=["xlsx", "xls"], key="hbh_main")
    ...
    if not all(required_files):
        ...
        with st.spinner("Crunching the numbers... Please wait..."):
            try:
                ...
                # Initialize variables that might be used across branches for final sum/display
                Enforce = 0.0
                sh_sum = 0.0
                net_calc = 0.0
                total_net = 0.0
                ...
                # Tier weights (only calculate if tier data is uploaded)
                if not pp.empty:
                ...
                dock = float(subtotal * 0.1)
                net = float(subtotal - dock) # This is 'Parking Net Revenue'
                ...
                st.balloons()
                st.success(f"🎉 Success! Report generated for {parking_code} on {target_date}.")
                ...
                m1.metric(f"{parking_code} Subtotal", fmt_currency(subtotal))
                m2.metric("External Rev (Sub/SH)", fmt_currency(net_calc + sh_sum))
                m3.metric("Enforcement Rev", fmt_currency(Enforce))
                m4.metric("Total Payout (Client)", fmt_currency(payout), f"{client_pct}% Split")

                st.markdown("### 📊 Financial Overview")
                # Create and Display Main Summary Table
                ...
                main_summary = pd.DataFrame({'Metric': metrics, 'Value': values})

                st.table(main_summary.style.format({"Value": "${:,.2f}"}))
                ...
                with pd.ExcelWriter(buffer, engine='openpyxl') as writer:
                    ...
                    main_summary.to_excel(writer, sheet_name='Report', index=False, startrow=row_idx)
                    row_idx += len(main_summary) + 3
                    ...
                    if parking_code == 'BCW' and 'bcws_all_rows' in locals() and not bcws_all_rows.empty:
                        ...
                        if 'Transaction Date' in bcws_export.columns:
                            parsed_dates_bcws = pd.to_datetime(bcws_export['Transaction Date'], errors='coerce')
                            bcws_export['Transaction Date'] = parsed_dates_bcws.dt.strftime('%Y-%m-%d %H:%M:%S').fillna(bcws_export['Transaction Date'])

                        sub_summary_data = {
                            'Metric': ['Sum of Total Amount', 'Sum of Payment Gateway Fee', 'Total (Sum of Total - Sum of Gateway)', 'Less Tech Fee (10% of Total)', 'Net (Total - Less Tech Fee)'],
                            'Value': [float(bcws_all_rows['Total Amount'].sum()), float(bcws_all_rows['Payment Gateway Fee'].sum()), total_calc, float(total_calc * 0.10), net_calc]
                        }
                        sub_summary = pd.DataFrame(sub_summary_data)

                        bcws_export.to_excel(writer, sheet_name='Report', index=False, startrow=row_idx)
                        row_idx += len(bcws_export) + 2
                ...
                st.download_button(
                    label=f"⬇️ Download {parking_code} Report (Excel)",
                    data=buffer.getvalue(),
                    file_name=f"{parking_code}_Payout_Report_{target_date}.xlsx",
