...
# --- Handle Logo Loading ---
# Use a relative path so it works on Streamlit Cloud; commit the image to the repo.
...
# --- Default Slice % per parking code (Client = 100 - Slice) ---
SLICE_DEFAULTS = {
    "BCW": 30,        # Baltimore Clayworks (and BCWS)
    "HBH": 30,        # HBH
    "GH": 30,         # Great Harvest
    "Gator": 30,      # Alligator Farm
    "TAJ": 35,        # Taj of India
    "CHS": 30,        # Church of Holy Spirit
    "UUMC": 30,       # UUMC
    "CP": 45,         # College Park (Upper lot)
    "Billiards": 35,  # Billiards
    "Dyson": 35,      # Dyson
    "BDCHP": 40,      # Baltimore Canton
    "Crain": 30,      # Crain Highway
    # Not in the provided table -> client-favored default
    "BlueWave": 30,
    "EastonDemo": 30,
    "SunBeach": 30,
}
...
# --- Callback functions (MUST be defined before the sliders that reference them) ---
def apply_code_defaults():
    """When the parking code changes, reset sliders to that lot's default split."""
    code = st.session_state.parking_code
    slice_val = SLICE_DEFAULTS.get(code, FALLBACK_SLICE)
    st.session_state.slice_pct = slice_val
    st.session_state.client_pct = 100 - slice_val
...
PARKING_CODES = ["BCW", "CP", "TAJ", "GH", "Gator", "HBH", "CHS", "BDCHP", "Billiards", "BlueWave", "Crain", "Dyson", "EastonDemo", "SunBeach", "UUMC"]

# --- Seed slider defaults from the FIRST code BEFORE any widget is created ---
# (Streamlit won't let you overwrite a slider-keyed value after that slider exists,
#  so the initial seed happens here and later changes go through apply_code_defaults.)
if 'slice_pct' not in st.session_state:
    first_slice = SLICE_DEFAULTS.get(PARKING_CODES[0], FALLBACK_SLICE)
    st.session_state.slice_pct = first_slice
    st.session_state.client_pct = 100 - first_slice

col0, col1, col2, col3 = st.columns(4)
with col0:
    parking_code = st.selectbox("🅿️ Parking Code", PARKING_CODES, key="parking_code", on_change=apply_code_defaults)
with col1:
    ...
    if not all(required_files):
        ...
        with st.spinner("Crunching the numbers... Please wait..."):
            try:
                ...
                payout = 0.0
                specific_sum_pp = 0.0
                tech = 0.0  # default so summary table never hits an undefined name
                ...
                if not pp.empty:
                    ...
                    pp_sum_series = pd.Series(dtype='float64')
                ...
                elif parking_code in ("CP", "CPUpperLot"):
                    subtotal = float(gross - credit)
                else:
                ...
                if parking_code == "BCW":
                    ...
                    sum_net_enf = float(filtered_er['Net'].sum()) if not filtered_er.empty else 0.0
                    specific_sum_pp = float(pp_sum_series.iloc[3]) if len(pp_sum_series) > 3 else 0.0  # Adjust index to match Parkpliant data
                    Enforce = float(sum_net_enf - specific_sum_pp)
                    ...
                    if not cp_upper_rows.empty:
                        total_calc = float(cp_upper_rows['Total Amount'].sum() - cp_upper_rows['Payment Gateway Fee'].sum())
                        net_calc = float(total_calc)
                    else:
                        total_calc = 0.0
                        net_calc = 0.0

                    tech = 350
                    total_net = float(net + net_calc + Enforce - tech)
                    payout = float(total_net * (client_pct / 100))

                elif parking_code == "TAJ":
                    filtered_er = er[er['Lot Code'] == 'TAJ']
                    sum_net_enf = float(filtered_er['Net'].sum()) if not filtered_er.empty else 0.0
                    specific_sum_pp = float(pp_sum_series.iloc[4]) if len(pp_sum_series) > 4 else 0.0  # Adjust index for TAJ
                    Enforce = float(sum_net_enf - specific_sum_pp)
                ...
                <div style='display: flex; gap: 20px; flex-wrap: wrap; margin-bottom: 20px;'>
                    ...
                    <div class='metric-card' style='flex: 1; min-width: 150px;'>
                        ...
                        <div class='metric-value'>{fmt_currency(tech_card_value)}</div>
                    </div>
                    <div class='metric-card' style='flex: 1; min-width: 150px;'>
                ...
                if parking_code == "BCW":
                    metrics = ['Gross Revenue', 'Service Fee', 'Credit Card Fee', 'City Tax', 'Subtotal', 'Tech Fee', 'Parking Net Revenue']
                    values = [gross, service, credit, tax, subtotal, dock, net]
                elif parking_code in ("CP", "CPUpperLot"):
                    metrics = ['Gross Revenue', 'Credit Card Fee', 'Subtotal', 'Tech Fee', 'Parking Net Revenue']
                    values = [gross, credit, subtotal, tech, net]
                else:
                    metrics = ['Gross Revenue', 'Service Fee', 'Credit Card Fee', 'Subtotal', 'Tech Fee', 'Parking Net Revenue']
                ...
                if parking_code == 'BCW':
                    ...
                    metrics.extend(['Parkpliant Revenue', 'Taggr Revenue', 'CP Upper Lot Net Revenue'])
                    values.extend([specific_sum_pp, Enforce, net_calc])
                elif parking_code == 'TAJ':
                ...
                with pd.ExcelWriter(buffer, engine='openpyxl') as writer:
                    ...
                    if parking_code == 'BCW' and 'bcws_all_rows' in locals() and not bcws_all_rows.empty:
                        ...
                        sub_summary_data = {'': ['Total Amount', 'Credit Card Fee', 'Service Fee', 'City Tax', 'Sub Total', 'Tech Fee', 'Net Total'], ' ': [''] * 7, '  ': [float(bcws_all_rows['Total Amount'].sum()), float(bcws_all_rows['Payment Gateway Fee'].sum()), float(bcws_all_rows['Service Fee'].sum()), float(bcws_all_rows['City Tax'].sum()), total_calc, float(total_calc * 0.10), net_calc]}
                        ...
                        sub_summary_data = {'': ['Total Amount', 'Credit Card Fee', 'Sub Total', 'Net Total'], ' ': [''] * 4, '  ': [float(cp_upper_rows['Total Amount'].sum()), float(cp_upper_rows['Payment Gateway Fee'].sum()), total_calc, net_calc]}
                        sub_summary = pd.DataFrame(sub_summary_data)
