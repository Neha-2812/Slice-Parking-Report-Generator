            ...
            try:
                ...
                with pd.ExcelWriter(buffer, engine='openpyxl') as writer:
                    ...
                    if parking_code == 'BCW' and 'bcws_all_rows' in locals() and not bcws_all_rows.empty:
                        ...
                        sub_summary_data = {'': ['Total Amount', 'Credit Card Fee', 'Service Fee', 'Sub Total', 'Tech Fee', 'Net Total'], ' ': [''] * 6, '  ': [float(cp_upper_rows['Total Amount'].sum()), float(cp_upper_rows['Payment Gateway Fee'].sum()), float(cp_upper_rows['Service Fee'].sum()), total_calc, float(total_calc * 0.10), net_calc]}
                        sub_summary = pd.DataFrame(sub_summary_data)
