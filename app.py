        ...
        with st.spinner("Crunching the numbers... Please wait..."):
            try:
                ...
                if parking_code == "BCW":
                    ...
                    filtered_er = er[er['Lot Code'] == 'Nebraska Lot - HBH']
                    sum_net_enf = float(filtered_er['Net'].sum()) if not filtered_er.empty else 0.0
                ...
                with pd.ExcelWriter(buffer, engine='openpyxl') as writer:
                    ...
                    main_summary_export.to_excel(writer, sheet_name='Report', index=False, header=False, startrow=row_idx)
                    ...
                    fill_color1 = PatternFill(start_color="E8F0FE", end_color="E8F0FE", fill_type="solid")
                    ...
                    if parking_code == 'BCW' and 'bcws_all_rows' in locals() and not bcws_all_rows.empty:
                        ...
                        sub_summary_data = {'': ['Sum of Total Amount', 'Sum of Payment Gateway Fee', 'Total (Sum of Total - Sum of Gateway)', 'Less Tech Fee (10% of Total)', 'Net (Total - Less Tech Fee)'], ' ': [''] * 5, '  ': [float(bcws_all_rows['Total Amount'].sum()), float(bcws_all_rows['Payment Gateway Fee'].sum()), total_calc, float(total_calc * 0.10), net_calc]}
                        ...
                        # Apply colors to BCWS rows
                        for row in range(bcws_start + 2, bcws_start + len(bcws_export) + 2):
                            fill = fill_color1 if row % 2 == 0 else fill_color2
                            for col in range(1, len(bcws_export.columns) + 1):
                                worksheet.cell(row=row, column=col).fill = fill

                        row_idx = bcws_start + len(bcws_export) + 2
                        sub_summary.to_excel(writer, sheet_name='Report', index=False, header=False, startrow=row_idx)

                        for row in range(row_idx + 1, row_idx + 1 + len(sub_summary)):
