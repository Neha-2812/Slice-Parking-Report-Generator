        ...
        with st.spinner("Crunching the numbers... Please wait..."):
            try:
                ...
                if parking_code == "BCW":
                    ...
                    cp_upper_rows = df[
                        (df['Parking Code'].astype(str).str.strip().str == 'CPUpperLot') &
                        (df['Payout Date'] == target_date_pd) &
                ...
                with pd.ExcelWriter(buffer, engine='openpyxl') as writer:
                    ...
                    if parking_code == 'BCW' and 'bcws_all_rows' in locals() and not bcws_all_rows.empty:
                        ...
                        sub_summary_data = {'': ['Sum of Total Amount', 'Sum of Payment Gateway Fee', 'Sum of Service Fee', 'Sum of City Tax', 'Sub Total', 'Tech Fee', 'Net Total'], ' ': [''] * 7, '  ': [float(bcws_all_rows['Total Amount'].sum()), float(bcws_all_rows['Payment Gateway Fee'].sum()), float(bcws_all_rows['Service Fee'].sum()), float(bcws_all_rows['City Tax'].sum()), total_calc, float(total_calc * 0.10), net_calc]}
                        ...
                        sub_summary_data = {'': ['Sum of Total Amount', 'Sum of Payment Gateway Fee', 'Total (Sum of Total - Sum of Gateway)', 'Tech Fee (10% of Total)', 'Net (Total - Less Tech Fee)'], ' ': [''] * 5, '  ': [float(cp_upper_rows['Total Amount'].sum()), float(cp_upper_rows['Payment Gateway Fee'].sum()), total_calc, float(total_calc * 0.10), net_calc]}
                        sub_summary = pd.DataFrame(sub_summary_data)
