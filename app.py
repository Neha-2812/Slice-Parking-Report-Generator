        ...
        with st.spinner("Crunching the numbers... Please wait..."):
            try:
                ...
                if parking_code == "BCW":
                    ...
                    cp_upper_rows = df[
                        (df['Parking Code'].astype(str).str.strip().str.upper() == 'CPUPPERLOT') &
                        (df['Payout Date'] == target_date_pd) &
                        (df['Purpose'].astype(str).str.strip().str.upper() == 'SUBSCRIPTION')
                    ]
                ...
                with pd.ExcelWriter(buffer, engine='openpyxl') as writer:
                    ...
                    if parking_code == 'BCW' and 'bcws_all_rows' in locals() and not bcws_all_rows.empty:
                        ...
                        sub_summary_data = {'': ['Sum of Total Amount', 'Sum of Payment Gateway Fee', 'Total (Sum of Total - Sum of Gateway)', 'Tech Fee (10% of Total)', 'Net (Total - Less Tech Fee)'], ' ': [''] * 5, '  ': [float(bcws_all_rows['Total Amount'].sum()), float(bcws_all_rows['Payment Gateway Fee'].sum()), total_calc, float(total_calc * 0.10), net_calc]}
