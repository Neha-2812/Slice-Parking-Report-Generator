        ...
        with st.spinner("Crunching the numbers... Please wait..."):
            try:
                ...
                if parking_code == "BCW":
                    ...
                    total_net = float(net + net_calc + Enforce - 350)
                    payout = float(total_net * (client_pct / 100))
                ...
                if parking_code == "BCW":
                  ...
                  tech = 350
                  values = [gross, service, credit, subtotal, tech, net]
                else:
                  metrics = ['Gross Revenue', 'Service Fee', 'Credit Card Fee', 'Subtotal', 'Tech Fee', 'Parking Net Revenue']
                ...
                with pd.ExcelWriter(buffer, engine='openpyxl') as writer:
                    ...
                    if parking_code == 'BCW' and 'bcws_all_rows' in locals() and not bcws_all_rows.empty:
                        ...
                        sub_summary_data = {'': ['Total Amount', 'Credit Card Fee', 'Service Fee', 'Sub Total', 'Net Total'], ' ': [''] * 5, '  ': [float(cp_upper_rows['Total Amount'].sum()), float(cp_upper_rows['Payment Gateway Fee'].sum()), float(cp_upper_rows['Service Fee'].sum()), total_calc, net_calc]}
                        sub_summary = pd.DataFrame(sub_summary_data)
