    ...
    if not all(required_files):
        ...
        with st.spinner("Crunching the numbers... Please wait..."):
            try:
                ...
                elif parking_code == 'CP' or parking_code == 'CPUpperLot':
                  if parking_code == 'CPUpperLot':
                    cpu_gross = 0.0
                    cpu_service = 0.0
                    cpu_credit = 0.0
                    cpu_gross = float(df_main['Total Amount'].sum()) if not df_main.empty else 0.0
                    cpu_service = float(df_main['Service Fee'].sum()) if not df_main.empty else 0.0
                    cpu_credit = float(df_main['Payment Gateway Fee'].sum()) if 'Payment Gateway Fee' in df_main.columns else 0.0
                    cpu_subtotal = float(cpu_gross - cpu_service - cpu_credit)
                  else:
                    cp_gross = 0.0
                    cp_service = 0.0
                    cp_credit = 0.0
                    cp_gross = float(df_main['Total Amount'].sum()) if not df_main.empty else 0.0
                    cp_service = float(df_main['Service Fee'].sum()) if not df_main.empty else 0.0
                    cp_credit = float(df_main['Payment Gateway Fee'].sum()) if 'Payment Gateway Fee' in df_main.columns else 0.0
                    cp_subtotal = float(cp_gross - cp_service - cp_credit)
                  subtotal = float(cpu_subtotal + cp_subtotal)
                else:
                ...
                if parking_code == "CP" or parking_code == "CPUpperLot":
                  if parking_code == 'CPUpperLot':
                ...
                if parking_code == "BCW":
                  ...
                  if parking_code == 'CPUpperLot':
                        filtered_er = er[er['Lot Code'] == 'CPUpperLot']
                        cpu_sum_net_enf = float(filtered_er['Net'].sum()) if not filtered_er.empty else 0.0
                        cpu_specific_sum_pp = float(pp_sum_series.iloc[3]) if len(pp_sum_series) > 3 else 0.0 # Assuming index 3 for CP
                        cpu_Enforce = float(cpu_sum_net_enf - cpu_specific_sum_pp)
                        cpu_total_net = float(cpu_net + cpu_Enforce)
                  else:
                    filtered_er = er[er['Lot Code'] == 'CP']
                    cp_sum_net_enf = float(filtered_er['Net'].sum()) if not filtered_er.empty else 0.0
                    cp_specific_sum_pp = float(pp_sum_series.iloc[3]) if len(pp_sum_series) > 3 else 0.0 # Assuming index 3 for CP, you might need to adjust based on exact Parkpliant data
                    cp_Enforce = float(cp_sum_net_enf - cp_specific_sum_pp)
                    cp_total_net = float(cp_net + cp_Enforce)
                  
                  total_net = float(cpu_total_net + cp_total_net)
                  payout = float(total_net * (client_pct / 100))
                ...
                if parking_code == "BCW":
                  ...
                  if parking_code == 'CPUpperLot':
                    ...
                    values = [cpu_gross, cpu_service, cpu_credit, cpu_subtotal, cpu_dock, cpu_net]
                  else:
                    metrics = ['Gross Revenue', 'Service Fee', 'Payment Gateway Fee', 'Subtotal', 'Tech Fee', 'Parking Net Revenue']
                    values = [cp_gross, cp_service, cp_credit, cp_subtotal, cp_dock, cp_net]
                else:
                  metrics = ['Gross Revenue', 'Service Fee', 'Payment Gateway Fee', 'Subtotal', 'Tech Fee', 'Parking Net Revenue']
                ...
                if parking_code == 'BCW':
                  ...
                  if parking_code == 'CPUpperLot':
                    ...
                    values.extend([cpu_specific_sum_pp, cpu_Enforce])
                  else:
                    metrics.extend(['Parkpliant Revenue', 'Taggr Revenue'])
                    values.extend([cp_specific_sum_pp, cp_Enforce])
                elif parking_code == 'TAJ':
                    metrics.extend(['Parkpliant Revenue', 'Taggr Revenue'])
                ...
                if parking_code == 'CP' or parking_code == 'CPUpperLot':
                  if parking_code == 'CPUpperLot':
                    metrics.extend(['CP_Upper_Lot Net Revenue'])
                    values.extend([cpu_total_net])
                  else:
                    metrics.extend(['CP Net Revenue'])
                    values.extend([cp_total_net])
                ...
                with pd.ExcelWriter(buffer, engine='openpyxl') as writer:
                    ...
                    if parking_code == 'CP' or parking_code == 'CPUpperLot':
                      bold_metrics = ['CP Gross Revenue', 'CP Subtotal', 'CP Parking Net Revenue', 'Total Net Revenue', 'CP Net Revenue', 'CP_Upper_Lot Net Revenue']
                    else:
                    ...
                    if parking_code == 'BCW' and 'bcws_all_rows' in locals() and not bcws_all_rows.empty:
                        ...
                        sub_summary_data = {'': ['BCWS Gross Revenue', 'Payment Gateway Fee', 'Service Fee', 'City Tax', 'BCWS Sub Total', 'Tech Fee', 'BCWS Net Total'], ' ': [''] * 5, '  ': [float(bcws_all_rows['Total Amount'].sum()), float(bcws_all_rows['Payment Gateway Fee'].sum()), float(bcws_all_rows['Service Fee'].sum()), float(bcws_all_rows['City Tax'].sum()), total_calc, float(total_calc * 0.10), net_calc]}
                        bold_metrics = ['BCWS Gross Revenue', 'Payment Gateway Fee', 'Service Fee', 'City Tax', 'BCWS Sub Total', 'BCWS Tech Fee', 'BCWS Net Total']
                    ...
                    if parking_code == 'CP' or parking_code == 'CPUpperLot':
                      if parking_code == 'CPUpperLot':
                        cp_export = df_export.drop(columns=[c for c in columns_to_drop if c in df_export.columns]).copy()
                        sub_summary_data = {'': ['CP_Upper_Lot Gross Revenue', 'Payment Gateway Fee', 'Service Fee', 'CP_Upper_Lot Sub Total', 'CP_Upper_Lot Net Revenue', 'CP_Upper_Lot Total Net Revenue'], ' ': ['']*5, '  ': [cpu_gross, cpu_service, cpu_credit, cpu_subtotal, cpu_dock, cpu_net, cpu_total_net]} 
                        bold_metrics = ['CP_Upper_Lot Gross Revenue', 'CP_Upper_Lot Sub Total', 'CP_Upper_Lot Net Revenue', 'CP_Upper_Lot Total Net Revenue'] 
                        sub_summary = pd.DataFrame(sub_summary_data)
                      else:
                        bold_metrics = ['CP Gross Revenue', 'CP Subtotal', 'CP Parking Net Revenue', 'Total Net Revenue', 'CP Net Revenue', 'CP_Upper_Lot Net Revenue']
